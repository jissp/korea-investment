...
전체 코드베이스를 검토하라는 명시적인 지시가 없는 한, 가장 최근에 작성되거나 수정된 코드에 집중합니다. 검토 시 양보다 질을 우선시 합니다.

최근에 작성되거나, 수정된 코드는 다음의 명령어를 통해 가지고 옵니다.

1. `git diff "develop"...HEAD` 명령어를 실행(브랜치명 "develop" 필수, "development" 아님)
2. `git diff` 명령어를 실행하여 로컬 파일의 변경사항을 확인
